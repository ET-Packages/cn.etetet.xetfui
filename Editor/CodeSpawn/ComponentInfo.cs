...
namespace FUIEditor
{
    public class VariableInfo
    {
        ...
        public bool CheckExportVariable()
        {
            if (FUICodeSpawner.IgnoreDefaultVariableName && IsDefaultName)
            {
                IsExported = false;
                return false;
            }
    ...
    public class ComponentInfo
    {
        ...
        public void SetVariableInfoTypeName()
        {
            for (int index = 0; index < this.VariableInfos.Count; index++)
            {
                ...
                variableInfo.CheckExportVariable();
            }
        }
        ...
        private string GetTypeNameByDisplayXML(string parentPackageId, XML displayXML)
        {
            ...
            if (displayXML.name == "component")
            {
                ...
                if (!FUICodeSpawner.ComponentInfos.ContainsKey(key))
                {
                    throw new Exception($"{this.ComponentTypeName}.{displayXML.GetAttribute("name")} 有问题，检查一下吧！");
                }
